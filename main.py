...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.CustomActivity(name="Vybing"))
...
async def play(ctx,url):
    try :
        ...
        friendlyFileName = filename.split('.')
        await ctx.send('**Now playing:** {}'.format(friendlyFileName[0]))
    except:
